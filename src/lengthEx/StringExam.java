...
public class StringExam {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		//""을 이용해서 생성된 문자열은 모두 String pool 에 저장되고 새로운 리터럴을 생성할시엔
		//먼저 pool 내에 같은 값이 있는 객체가 존재하는지 찾아본다. 만약 있다면 해당 Ref를 리턴합니다.
		//이 조건은 오직 ""(리터럴) String 객체에만 적용됩니다.
		...
		if(a == b) {
			System.out.println("같은 객체니 값도 당연히 같습니다.");
		}else {
			System.out.println("틀린 객체이고, 값이 같은지는 모르겠습니다.");
		}
		//String 의 override 된 equals 사용. 두 문자열 객체가 같은 값이면 true 아님 false 리턴. 
		if(c.equalsIgnoreCase(d)) {
			System.out.println("다른 문자열 객체지만 값은 같음.");
		}else{
			System.out.println("다른 문자열 객체이며 값도 다름.");
		}
		 ...
		 * 
		 * 얘를 호출해서 바이트 배열로 변환 후 , 배열의 모든 값을 다시 새로운 문자열로 생성하는데,
		 * 단 , 문자열의 캐릭터셋을 ISO88591 로 되어진 애로 변환해서 출력해보세요.
		 */
		...
		//문자열의 끝과 처음 판별하기
		String email = "dlawlsdn@naver.com";
		if(email.endsWith(".com") || email.endsWith("co.kr")){
			System.out.println("올바른 서버명입니다.");
		}
		
		//format() : static 이면서, 문자열,수치형등을 모두 특정 형식대로 포맷하여 설정 할 수 있습니다.
		/*
		 * 아래는 간단한 문법입니다.
		 * 
		 * %d(10진수 형식),%s(문자열 형식), %f(실수형), Locale(날짜등의 형식)
		 * 기본적으로 포맷을 적용하면 우측 정렬이 적용됩니다. 만약 -표시를 하게 되면 왼쪽 정렬을 하게 됩니다.
		 */
		int i = 23;
		...
		//여러분의 이메일을 서버명,green.com 으로 변경해보세요
