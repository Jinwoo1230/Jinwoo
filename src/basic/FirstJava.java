 ...
 * 이름은 반드시 * 10000000000 첫자는 대문자로 시작해야하고, 한 단어 이상 연결시
 * 새 단어의 첫 단어는 기본적으로 대문자입니다. "-"(언더바)와 $ 를 사용할 수 있지만
 * 아무도 사용하지 않습니다.
 * 
 * 모든 변수, 클래스, 메서드 이름에는 첫 문자는 숫자가 올 수 없습니다.
 */
class FirstJava {//클래스 body
		public static void main(String[] args) {//메서드 body
			
			byte a,b; //a 지역 변수 선언(declared)(메서드내에서 처음 선언된 변수를 지연변수라 함.)
					 //메인에서 byte 타입으로 사용하기 위한 a,b선언
			a = 1;  //Assign
			b = 1;
			byte c = 1;
			...
			/*
			 * 자바에서는 지역변수를 사용할 때 반드시 값이 초기화 되어 있어야 합니다.
			 * 위에서 초기화란 초기화 또는 값이 존재해야 한다는 말입니다.
			 * 변수를 처음 선언할 때는 반드시 변수명 앞에 타입이 선언되어야 합니다.
			 * 이렇게 선언된 변수는 해당 타입만 값을 넣을 수 있습니다.
			 */
			
			//리터널데이터 : 값 자체가 어떤 타입인지를 내포하고 있는 데이터.
			//10 + 20 수치형(정수) <-- 값 자체가 변수에 담기지 않고 코드에 나오는 경우 : 리터널데이터
			//"안녕" <-- 문자열 : 리터널데이터
