...
public class First {
	...
	//이 생성자는 protected 이기에 같은 패키지에서는 호추가능, 다른 패키지에선 오로지 자식 클래스만 호출 가능함.
...
/*
 * 접근제어자(AccessModifier) : 접근제어자는 public > protected > default > private 순으로 사용할 수 있습니다.
 * 사용 범위는 클래스(public or default) 필드, 생성자, 메서드 (4개모두)
 * 접근제어자를 선언하지 않을시 컴파일러는 기본적으로 default(이름없는) 접근제어자를 넣어줍니다.
 * 
 * public 은 패키지로 구분없이 모두에게 참조 가능한 형태를 뜻하며
 * protected 는 같은 패키지내에선 접근 가능, 다른 패키지에서는 상속받은 자식 클래스만 가능합니다.
