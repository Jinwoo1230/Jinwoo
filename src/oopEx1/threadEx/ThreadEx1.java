 ...
 * 
 * 위 3개중 뭐가 됐든, 반드시 run() 을 오버라이드 해야하고, Runnable 상태로 전이 되기 위해선 반드시 start() 를 호출해야한다.
 */

class ThreadEx1_1 extends Thread{//쓰레드를 상속했기에 이 클래스자체가 쓰레드다. 
...
class ThreadEx1_2 extends Thread{//쓰레드를 상속했기에 이 클래스자체가 쓰레드다. 
	//수행 로직을 run() 을 통해 구현한다.
	@Override
	public void run() {
		for(int i = 0; i<5; i++) {
