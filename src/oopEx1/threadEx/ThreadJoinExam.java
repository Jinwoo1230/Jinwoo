 ...
 * 
 * sleep(), join(), yield() ... 나머지 메서드는 resume() 등.. 모두 deprecated 되었기 때문에 사용되지 않는다.
 */
...
public class ThreadJoinExam {
	...
	public static void main(String[] args) {
		/*
		 * 이 메인 쓰래드에서는 위의 Join 쓰래드를 start() 시킨후
		 * 1 ~ 100 까지의 연산 합을 리턴받을 예정입니다.
		 * join() 을 호출하기 전에는 연산이 완료되기전에 호출이 되어서 완전한 값을 get 하지 못합니다.
		...
		try {
			...
			//이 때 main 쓰래드는 누가 깨워주지 않으면, 계속 실행대기 영역에서 머무르게 되기 때문에 문제가 발생합니다.
			//따라서 내부적으로 InterruptedException을 발생시켜 main 쓰래드를 깨워, Runnable 상태로 다시 갈 수 있도록 만드는겁니다.
			threadB.join();
