		...
		try {
			while (true) {
				...
				if (input.equalsIgnoreCase("q")) {
					...
					System.out.println("history : 최근 입력 명령어를 최대 " + MAX_INPUT_SIZE + " 개 까지 보여줍니다.");
					...
					// 아래는 조금있다가 배울 Iterable(기억나세요 ???) 로 변환해서 Q에 있는 명령어를 추출 하도록 정의합니다.
					LinkedList<String> temp = (LinkedList<String>) q;
					ListIterator<String> it = temp.listIterator();
				
					// Iterator 타입(내부 순환자) 의 대표적 메서드로 요소가 존재하면 true 리턴
					// 이걸 이용하면 요소의 끝까지 리턴받을 수 있음.
					while (it.hasNext()) {
			...
			System.out.println("왜 그딴식으로 치니.. 뒤질래 ? 똑바로 쳐.");
		}
	}// End of main
