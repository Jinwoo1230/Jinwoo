 ...
 * 멤버충접, 정적(static) 중첩, 지역 중첩, 익명 중첩
 * 
 * 이렇게 중첩클래스 OR 중첩 인터페이스를 두는 이유는 특정 클래스가 외부에 노출되어 어떤 일을 하기보다는, 특정 클래스에서만 사용되어야 하고
 * 또한, 코드의 복잡성을 내부에 감추어서 사용하기 편하도록 하는데 있습니다.
 * 
 * 1.멤버중첩클래스 : 이 클래스는 위치가 내부 클래스의 멤버 필드 위치로 선언되어있는 형태입니다.
 * 사용목적으론 외부 클래스의 인스턴스 멤버처럼 활용하는데 있고, 주로 외부 클래스의 멤버들과 관련해서 작업을 하는데 사용됩니다.
 */
class InnerEx1{
	int a = 10;
	private int b = 100;
...
public class Inner1Ex {
	...
	public static void main(String[] args) {
		//Inner 클래스의 인스턴스를 생성시엔 Outer.Inner 타입으로 생성해야 합니다 꼭 기억하세요.
